...
export async function runMigrations() {
    ...
    try {
        // Obtain a dedicated client for transaction from the pool
        client = await pool.connect();
    } catch (err) {
        console.error('❌ Failed to acquire connection from pool for migrations:', err.message);
        throw err;
    }
    ...
    try {
        ...
        try {
            ...
            if (migErr.code === '42P01') {
                ...
                throw migErr; // Re-throw to be caught by outer catch for ROLLBACK
            }
        }
        ...
        try {
            ...
            const fromIdCheck = await client.query(`
                SELECT column_name FROM information_schema.columns 
                WHERE table_name = 'recipes' AND column_name = 'shared_from_id'
            `);
            if (fromIdCheck.rows.length === 0) {
                await client.query(`ALTER TABLE recipes ADD COLUMN shared_from_id INTEGER REFERENCES recipes(id) ON DELETE SET NULL`);
                console.log('✅ shared_from_id column added to recipes');
                migrationCount++;
            }

            // Check for shared_from_store_id
            const storeIdCheck = await client.query(`
                SELECT column_name FROM information_schema.columns 
                WHERE table_name = 'recipes' AND column_name = 'shared_from_store_id'
            `);
            if (storeIdCheck.rows.length === 0) {
                await client.query(`ALTER TABLE recipes ADD COLUMN shared_from_store_id INTEGER REFERENCES store_recipes(id) ON DELETE SET NULL`);
            ...
            if (notesCheck.rows.length === 0) {
                await client.query(`ALTER TABLE recipes ADD COLUMN shared_notes TEXT`);
        ...
        try {
            const expiryTypeCheck = await client.query(`
                SELECT data_type FROM information_schema.columns 
            ...
            if (expiryTypeCheck.rows.length > 0 && expiryTypeCheck.rows[0].data_type !== 'bigint') {
                await client.query(`ALTER TABLE users ALTER COLUMN reset_code_expiry TYPE BIGINT USING EXTRACT(EPOCH FROM reset_code_expiry)::BIGINT * 1000`);
        ...
        try {
            const passNullCheck = await client.query(`
        ...
        if (migrationCount > 0) {
            console.log(`\n✨ ${migrationCount} migration(s) applied successfully in ${elapsed}ms`);
        } else {
            console.log(`\n✅ Database is up to date (checked in ${elapsed}ms)`);
        }
    } catch (err) {
        // Rollback transaction on any critical error
        console.error('❌ Migration error occurred, rolling back changes...', err.message);
        try {
            if (client) {

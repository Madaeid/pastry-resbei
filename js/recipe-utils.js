...
export function getCategoryEmoji(category) {
    ...
    return emojis[category?.toLowerCase()] || emojis[category] || '🧁';
}
...
export function getDifficultyText(difficulty) {
    ...
    return levels[difficulty?.toLowerCase()] || levels[difficulty] || '🟡 Medium';
}

export function formatRecipeTime(prepTime, cookTime) {
 ...
 */
export function createPublicRecipeCard(recipe) {
    ...
    let authorName = recipe.author?.name || recipe.authorName || 'Chef';
    ...
    card.onmouseenter = () => { card.querySelector('.overlay-actions').style.opacity = '1'; };
    card.onmouseleave = () => { card.querySelector('.overlay-actions').style.opacity = '0'; };
    ...
    menuBtn.onclick = (e) => {
        e.stopPropagation();
        if (typeof window.openDayPickerModal === 'function') {

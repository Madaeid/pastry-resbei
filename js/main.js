...
import '../css/nutrition-analyzer.css';
            ...
            addRecipeForm.onsubmit = async (e) => {
                ...
                if (!token) {
                    ...
                    if (confirm(data.message + '\n\nWould you like to upgrade to Premium now?')) {
                        window.location.href = 'pages/payment.html?source=book';
                    } return;
                }
                ...
                try {
                    ...
                    if (response.ok) {
                        if (data.requiresPremium) {
                            ...
                            setTimeout(() => window.location.href = 'pages/payment.html', 2000);
                            return;
                        }
    ...
    modal.innerHTML = `
        <div class="modal-content modal-sm premium-upgrade-modal">
            ...
            <div class="modal-actions">
                ...
                <button class="btn btn-primary btn-premium" onclick="window.location.href='./pages/payment.html'">
                    ⭐ Upgrade Now
                </button>
            </div>
        </div>
    ...
    if (userHeader && userNameElement) {
        ...
        if (userProfilePic && userDefaultAvatar) {
            ...
            userProfilePic.onclick = () => {
                window.location.href = `./pages/chef-profile.html?username=${currentUsername}`;
            };
        }
...
function renderHomeSearchResults(users, dropdown) {
    if (!users || users.length === 0) {
        ...
        dropdown.innerHTML = users.slice(0, 8).map(user => `
            <div class="search-result-item" onclick="window.location.href='./pages/chef-profile.html?username=${user.username}'">
...
function createChefCard(chef) {
    ...
    viewBtn.onclick = (e) => {
        e.stopPropagation();
        window.location.href = `pages/chef-profile.html?username=${chef.username}`;
    };
...
async function handleShareRecipe(recipe) {
    ...
    const url = `${window.location.origin}/pages/chef-profile.html?username=${username}&post=${recipe.id}`;
    const token = sessionStorage.getItem('authToken');
    ...
    modal.innerHTML = `
        <div class="modal-content upgrade-modal-content">
            ...
            <div style="display: flex; gap: 15px; justify-content: center;">
                ...
                <a href="./pages/payment.html?source=${source}" class="btn btn-upgrade" style="text-decoration: none;">
    ...
    modal.innerHTML = `
        <div class="modal-content premium-profile-modal">
            ...
            <div class="premium-card-info">
                ...
                <button class="btn-manage-sub" onclick="window.location.href='./pages/payment.html'">
